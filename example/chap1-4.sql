...
SELECT
    ...
    -- REPLACE(~, '원본', '변경') 변경이 없으면 찾아서 삭제하는 용도로 사용
    REPLACE(p.content, '#일상', '#데일리') AS "해시태그 교체",
    p.CONTENT AS "원본 게시글"
FROM
    USERS u, POSTS p
WHERE
    u.user_id = p.user_id AND p.CONTENT LIKE '%일상%'
;
...
SELECT
    ...
    FLOOR(3.14) AS "인기점수 (내림)", -- FLOOR 자리수 지정 X
    MOD(user_id, 2) AS "짝홀 구분 (0이면 짝수, 1이면 홀수)" -- % 연산
FROM
    USERS
WHERE
...
SELECT *
FROM POSTS
WHERE creation_date > TO_DATE('240520', 'YYMMDD'); -- '>': TO_DATE() todate 이후 날짜인지 판단
